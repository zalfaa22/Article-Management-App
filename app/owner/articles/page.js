        ...
        <div className='p-6'>
          ...
          <div className='bg-white shadow-md rounded-lg p-6'>
            {!selectedArticle ? (
                ...
                <div className='flex justify-center mt-4'>
                  {Array.from({ length: totalPages }, (_, index) => (
                    <button
                      ...
                      className={`mx-1 px-3 py-1 border rounded ${currentPage === index + 1 ? 'bg-blue-500 text-white' : 'bg-white text-gray-700'}`}
                    >
                      {index + 1}
                    </button>
                  ))}
              ...
              <div>
                <button onClick={() => setSelectedArticle(null)} className='mb-4 text-blue-500'>Back to Articles</button>
                <div className='border rounded-lg overflow-hidden shadow-lg'>
                  <img src={selectedArticle.image} alt={selectedArticle.title} className='w-full h-64 object-cover' />
                  <div className='p-4'>
                    <h2 className='text-lg font-bold mb-3 mt-2'>{selectedArticle.title}</h2>
