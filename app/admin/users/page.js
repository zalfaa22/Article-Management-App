...
export default function UsersPage() {
  ...
  const handleEdit = (id) => {
    // Menetapkan data user yang akan diedit ke dalam state
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <button className='mr-6 bg-yellow-300 px-3 py-1' onClick={() => handleEdit(row.id)}>Edit</button>
          <button className='bg-red-500 px-3 py-1 text-white' onClick={() => handleDelete(row.id)}>Delete</button>
        </div>
      ),
    },
  ];
